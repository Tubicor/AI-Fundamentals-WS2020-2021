...
class FuzzyPlayer(Player):
    def __init__(self, racket: Racket, ball: Ball, board: Board):
        ...
        x_dist = fuzz.control.Antecedent(range(-800,800),"x_dist")
        edge = 800
        farnormal = 35
        halfCenter = 5
        safeCenter = 0
        ...
        rules.append(fuzz.control.Rule(x_dist["right"] & y_dist["close"] ,speed["stay"]))
        rules.append(fuzz.control.Rule(x_dist["farRight"] & y_dist["close"] ,speed["goFarRight"]))

        controller = fuzz.control.ControlSystem(rules)
        self.racket_controller = fuzz.control.ControlSystemSimulation(controller)
    ...
    def make_decision(self, x_diff: int, y_diff: int):
        ...
        self.racket_controller.input["racket_position"] = self.racket.rect.x
